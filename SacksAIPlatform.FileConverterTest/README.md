...
### ?? Console vs Debug Output
By default, all logging is redirected to the **Visual Studio Debug Output Window** to prevent interference with console user interaction.

You can control logging output via `appsettings.json`:

```json
{
  "Logging": {
    "RedirectToDebug": true,     // Send logs to Debug window
    "EnableConsoleLogging": false // Avoid console interference
  }
}
```

**Recommended Settings:**
- **During Development**: `"RedirectToDebug": true, "EnableConsoleLogging": false`
- **For Debugging**: Enable both if you need logs in console
- **Production**: Use file logging only

### ?? Viewing Debug Logs
1. **Visual Studio**: View ? Output ? Show output from: Debug
2. **VS Code**: Debug Console when debugging
3. **File Logs**: Always available in `logs/fileconverter-test-*.txt`

This ensures clean console interaction while maintaining full logging capabilities for troubleshooting.